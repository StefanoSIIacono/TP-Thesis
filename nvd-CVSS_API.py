...
class NVDAsyncClient:
    def __init__(self, api_key: Optional[str] = None):
        """
        Inizializza il client asincrono per NVD
    ...
    async def fetch_cve_details(self, cve_id: str) -> Optional[Dict[str, Any]]:
        """
        Recupera i dettagli asincroni per un singolo CVE
        
        :param cve_id: ID del CVE da recuperare
        :return: Dizionario con i dettagli del CVE
        """
        headers = {
            'User-Agent': 'AsyncCVERetriever/1.0',
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    return {
                        'CVE_ID': cve_id,
                        'Raw_Vulnerability_Data': vulnerability,
                        'CVSS_Scores': {
                            'Base_Score': cvss_data.get('cvssData', {}).get('baseScore'),
                            'Severity_Level': cvss_data.get('baseSeverity'),
                        ...
                        'CVSS_Metrics': {
                            ...
                            'Scope': cvss_data.get('cvssData', {}).get('scope'),
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Uso: python script.py CVE-XXXX-XXXXX [CVE-YYYY-YYYYY ...]")
        sys.exit(1)
